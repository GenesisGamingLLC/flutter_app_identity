...
/// Renames the Android app names and labels in the strings.xml and
/// AndroidManifest.xml files.
///
/// This function creates or updates the `res/values/strings.xml` file with
/// the app name, launcher name, and activity name from the [config].
///
/// It also updates the AndroidManifest.xml to reference these string
/// resources for labels.
///
/// In dry-run mode, the files are analyzed and the intended changes are
/// logged, but no files are created, modified, renamed, or deleted.
///
/// [config] - The configuration containing the new app names.
void renameAndroidNames(RenameConfig config) {
  // ---------------------------------------------------------------------------
  // 1. Ensure res/values/strings.xml exists
  // ---------------------------------------------------------------------------
  ...
  if (!valuesDir.existsSync()) {
    DryRun.createDirectory(
      valuesDir,
      description: 'Create Android resource directory: ${valuesDir.path}',
    );
  }
...
''';
  ...
  DryRun.writeFile(
    stringsFile,
    stringsContent,
    description: 'Update Android app names in ${stringsFile.path}',
  );

  if (DryRun.enabled) {
    Logger.info(
      '[DRY-RUN] Android strings.xml would be created/updated',
    );
  } else {
  ...
  // ---------------------------------------------------------------------------
  // 2a. Update <application> android:label
  // ---------------------------------------------------------------------------
  ...
  DryRun.writeFile(
    manifestFile,
    content,
    description: 'Update AndroidManifest.xml labels',
  );

  if (DryRun.enabled) {
    Logger.info(
      '[DRY-RUN] AndroidManifest.xml would be updated with labels',
    );
  } else {
    Logger.success(
      'AndroidManifest.xml updated with labels and namespace',
    );
  }
}
